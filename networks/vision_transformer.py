...
from networks.swin_transformer_unet_skip_expand_decoder_sys import SwinTransformerSys, MiMSwinTransformerSys 
...
class SwinUnet(nn.Module):
    ...
    def forward(self, x, feat=False):
        ...
        logits, x_feat = self.swin_unet(x)
...
class MiMSwinUnet(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(MiMSwinUnet, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.config = config
        embed_dim = config.MODEL.SWIN.EMBED_DIM

        self.swin_unet = MiMSwinTransformerSys(img_size=config.DATA.IMG_SIZE,
                                patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                in_chans=config.MODEL.SWIN.IN_CHANS,
                                num_classes=self.num_classes,
                                embed_dim=config.MODEL.SWIN.EMBED_DIM,
        ...
        for class_c in range(num_classes):
            ...
            # print(selector[0].weight)
            # print(selector[0].bias)
            
            # print(selector[0].weight.shape)
            # print(selector[0].bias.shape)
